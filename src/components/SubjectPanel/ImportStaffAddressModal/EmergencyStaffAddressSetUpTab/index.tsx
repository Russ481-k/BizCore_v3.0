...
interface EmergencyStaffAddressSetUpTabProps {
  setOpen: (isOpen: boolean) => void;
}

function EmergencyStaffAddressSetUpTab({
  setOpen,
}: EmergencyStaffAddressSetUpTabProps) {
  ...
  const {
    ...
    emergencyStaffAddressGroup: string;
    emergencyStaffAddressGroupName: string;
  }>({ mode: "onBlur" });
  ...
  const handleUpdateEmergencyStaffAddress = handleSubmit(
    ({ emergencyStaffAddressGroup, emergencyStaffAddressGroupName }) => {
      console.log(
        emergencyStaffAddressGroup,
        emergencyStaffAddressGroupName,
        addressData
      );
    }
  );
  ...
  return (
    <Flex gap={3} height="640px">
      <CollapseSection flex={3} flexDirection="column" headerTitle="조직도">
        <Flex flexDirection="column" height="580px" overflowY="auto">
          <Button
            alignItems="left"
      ...
      >
        ...
        >
          <TipText
            size="sm"
            text={`수정하려는 비상 연락망 그룹을 선택하세요.\n[새 비상 연락망 그룹]을 선택하여 추가할 수 있습니다.`}
          />
        </Flex>
        <Flex>
          <InfoBox>
            <InfoElement label="그룹" labelWidth="90px">
              <CustomSelect
                size="sm"
                {...register("emergencyStaffAddressGroup")}
              />
            </InfoElement>
          </InfoBox>
        </Flex>
        ...
        >
          <TipText
            size="sm"
            text="비상 연락망 그룹명을 수정하거나 구성원을 추가 또는 제외하신 다음 [저장] 버튼을 클릭하세요."
          />
        </Flex>
        <Flex>
          <InfoBox>
            <InfoElement label="그룹명" labelWidth="90px" required>
              <Input
                isInvalid={!!errors.emergencyStaffAddressGroupName}
                size="sm"
                {...register("emergencyStaffAddressGroupName", {
                  required: true,
                })}
              />
            </InfoElement>
        ...
        <Flex flexDirection="column" gap={3} height="300px" overflowY="hidden">
        ...
        <Flex justifyContent="space-between">
          ...
          <Button
            isDisabled={!addressData.length}
            size="sm"
            type="submit"
            variant="secondaryBlue"
            width="180px"
            onClick={handleUpdateEmergencyStaffAddress}
          >
            비상 연락망 그룹 저장
          </Button>
        </Flex>
...
export default EmergencyStaffAddressSetUpTab;
