...
function AddDeptModal({ isOpen, onRefetch, setModalOpen }: AddDeptModalProps) {
  ...
  const { mutate: addDept } = useAddDept();
    ...
    addDept(
      ...
      },
      {
        onError: () => {
          toast({
            render: () => (
              <ToastMessage title="부서 추가 오류" type="ERROR">
                부서 추가 중 오류가 발생하였습니다.
                <br />
        ...
        onSuccess: () => {
          toast({
            render: () => (
              <ToastMessage title="부서 추가 완료" type="SUCCESS">
                부서를 정상적으로 추가하였습니다.
              </ToastMessage>
            ),
          });
    ...
    <>
      <CustomModal isOpen={isOpen} onClose={onClose}>
        <ModalContent as="form" minW="680px">
          <ModalHeader>부서 추가</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Flex flexDirection="column" gap={1}>
              {/* 부서명 입력과 관리부서를 선택하신 다음 [추가] 버튼을 클릭하세요. */}
              <TipText
                hasBg
                text="부서명을 입력하신 다음 [추가] 버튼을 클릭하세요."
              />
              <InfoBox>
...
export default React.memo(AddDeptModal);
