...
function ImmediateSendModal({ sendData, onClose, onReset }: ImmediateSendModalProps) {
    ...
    sendMessage(
      ...
      },
      {
        onError: (error) => {
          toast({
            render: () => (
              <ToastMessage title="즉시  오류" type="ERROR">
                <Text>문자 메시지 즉시 중 알 수 없는 오류가 발생하였습니다</Text>
                <Text>
                  즉시 을 클릭하여 문자 메시지 즉시 을 다시 진행 하세요. 본 오류가 계속 발생하는
        ...
        onSuccess: () => {
          toast({
            render: () => (
              <ToastMessage title="문자 메시지 즉시  완료" type="SUCCESS">
                문자 메시지의 즉시 을 완료하였습니다. 된 문자 메시지는 메시지 관리의 전송 현황
                페이지에서 확인하세요.
              </ToastMessage>
            ),
            duration: 3000,
        ...
        <ModalBody>
          <Flex>
            ...
            <Flex flexDirection="column">
              <Text fontSize="14px">문자 메시지를 즉시 하면 취소할 수 없습니다.</Text>
