...
import { useDeleteTemplateGroup } from "features/sopp";
      ...
      deleteTemplateGroup(
        ...
        },
        {
          onError: () => {
            toast({
              render: () => (
                <ToastMessage title=" 그룹 삭제 오류" type="ERROR">
                  그룹 삭제 중 알 수 없는 오류가 발생하였습니다.
                  <br />
                  그룹 삭제를 다시 진행 하세요. 본 오류가 계속 발생하는 경우
                  시스템 관리자에게 문의하기 바랍니다.
                </ToastMessage>
              ),
            });
          ...
          onSuccess: () => {
            toast({
              render: () => (
                <ToastMessage title=" 그룹 삭제 완료" type="SUCCESS">
                  그룹을 정상적으로 삭제하였습니다.
                </ToastMessage>
              ),
            });
  ...
  return (
    <CustomModal isOpen onClose={onClose}>
      <ModalContent minW="420px">
        <ModalHeader> 그룹 삭제</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          {groupTemplateId === 0 ? (
            <Flex flexDirection="column" gap={1}>
              <Text fontSize="14px">
                템플릿 그룹에 속한 이 있으므로 삭제할 수 없습니다.
              </Text>
              <Text fontSize="14px">
                템플릿 그룹에 속한 을 삭제한 후 다시 시도하세요.
              </Text>
            </Flex>
          ) : (
            <Flex flexDirection="column" gap={1}>
              <Text fontSize="14px">삭제된 그룹은 복구가 불가합니다.</Text>
              <Text fontSize="14px">선택한 그룹을 삭제하시겠습니까?</Text>
            </Flex>
          )}
        </ModalBody>
        <ModalFooter gap={2}>
