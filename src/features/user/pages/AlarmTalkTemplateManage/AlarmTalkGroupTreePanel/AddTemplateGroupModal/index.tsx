...
function AddTemplateGroupModal({ onClose }: AddTemplateGroupModalProps) {
  ...
  const { mutate: addAlarmTalkTemplateGroup, isLoading } =
    useAddAlarmTalkTemplateGroup();
      ...
      addAlarmTalkTemplateGroup(
        ...
        },
        {
          onError: (error) => {
            toast({
              render: () => (
                <ToastMessage title="알림톡 템플릿 그룹 등록 오류" type="ERROR">
                  {error.message}
                </ToastMessage>
              ),
            });
          ...
          onSuccess: () => {
            toast({
              render: () => (
                <ToastMessage
                  title="알림톡 템플릿 그룹 등록 완료"
                  type="SUCCESS"
                >
                  알림톡 템플릿 그룹 등록이 완료되었습니다.
                </ToastMessage>
              ),
            });
            onClose();
  ...
  return (
    <CustomModal isOpen onClose={onClose}>
      <ModalContent as="form" minW="680px">
        <ModalHeader>알림톡 템플릿 그룹 추가</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Flex flexDirection="column" gap={1}>
            <TipText
              hasBg
              text="알림톡 템플릿 그룹명을 입력 하신 다음 [추가] 버튼을 클릭하세요."
            />
            <InfoBox>
              <InfoElement label="알림톡 템플릿 그룹명" required>
