...
function ChangeTemplateGroupModal({
  ...
  const { mutate: changeTemplateGroup, isLoading } = useChangeTemplateGroup();
      ...
      changeTemplateGroup(
        ...
        },
        {
          onError: (error) => {
            toast({
              render: () => (
                <ToastMessage title=" 그룹 수정 오류" type="ERROR">
                  {error.message}
                </ToastMessage>
              ),
            });
          ...
          onSuccess: () => {
            toast({
              render: () => (
                <ToastMessage title=" 그룹 수정 완료" type="SUCCESS">
                  그룹 수정이 완료되었습니다.
                </ToastMessage>
              ),
            });
  ...
  return (
    <CustomModal isOpen onClose={onClose}>
      <ModalContent as="form" minW="680px">
        <ModalHeader> 그룹 수정</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Flex flexDirection="column" gap={1}>
            <TipText
              hasBg
              text=" 그룹명을 수정 입력 하신 다음 [수정] 버튼을 클릭하세요."
            />
            <InfoBox>
